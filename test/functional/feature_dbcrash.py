...
class ChainstateWriteCrashTest(Bitcoin69TestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            ...
            try:
                ...
                # If btc69d exits, then try again.  wait_for_node_exit()
                # should raise an exception if btc69d doesn't exit.
                self.log.debug("Wait for node exit ~~ during restart, node: %s", node_index)
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, btc69d isn't coming back up on restart.  Could be a
        # bug in btc69d, or we've gotten unlucky with our dbcrash ratio --
