...
The RPM spec file provided here is for Bitcoin69-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://btc69crypto.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin69 git repository](https://github.com/JustAResearcher/Bitcoin69) but are not part of
...
Currently only `Source10` is used, for the example `btc69.conf` file.
...
in the btc69 git repository. At present only an SVG version of the Bitcoin69
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the btc69
...
needed when 0.12.1 is released, a proper fix is already in the Bitcoin69 git
master branch.
...
idea to build Bitcoin69 Core against a static version of that library compiled
...
The `btc69.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `btc69-server` package. At this point, most RPM based
...
The `btc69-server` package should have SELinux support. How to properly do
...
Bitcoin69-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/btc69/). He has been
packaging Bitcoin69 for Fedora at least since 2012.
...
4. I renamed the `btc69` package that contains the Qt GUI to `btc69-core` as
that appears to be how the general population refers to it, in contrast to
`btc69-xt` or `btc69-classic`. I wanted to make sure the general population
