...
git clone https://github.com/JustAResearcher/Bitcoin69
mkdir -p release
cd btc69coin/depends
...
sudo find ${DISTNAME}/bin -type f -executable -exec ../btc69coin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../btc69coin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo btc69-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find btc69-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/btc69coin
...
cp -f btc69-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf btc69coin/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/btc69coin/btc69-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/btc69-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer btc69-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer btc69-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf btc69coin/contrib/windeploy ~/sign/win32
...
#transfer btc69-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/btc69coin/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/JustAResearcher/Bitcoin69
mkdir ~/btc69coin/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder btc69coin/depends/SDKs
cd btc69coin/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/btc69coin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Bitcoin69-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/btc69-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer btc69-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
