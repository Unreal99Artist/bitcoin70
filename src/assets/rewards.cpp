...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for btc69
    ...
    if (p_rewardSnapshot.strDistributionAsset != "btc69") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is btc69 with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using btc69 differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "btc69") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Bitcoin69 address (already validated during ownership snapshot creation)
