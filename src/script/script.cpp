...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** BTC69 START */
    case OP_btc69_ASSET              : return "OP_btc69_ASSET";
    /** BTC69 END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_btc69_ASSET) { // OP_btc69_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == btc69_N) { // Check to see if btc69 starts at 27 ( this->size() < 105)
                if ((*this)[28] == btc69_E)
                    if ((*this)[29] == btc69_X)
                        index = 30;
            } else {
                if ((*this)[28] == btc69_N) // Check to see if btc69 starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == btc69_E)
                        if ((*this)[30] == btc69_X)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == btc69_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == btc69_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == btc69_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == btc69_N) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_btc69_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_btc69_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_btc69_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_btc69_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_btc69_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_btc69_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
