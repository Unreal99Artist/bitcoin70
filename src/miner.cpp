...
void static Bitcoin69Miner(const CChainParams& chainparams)
{
    LogPrintf("Bitcoin69Miner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("btc69-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("Bitcoin69Miner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Bitcoin69Miner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Bitcoin69Miner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("Bitcoin69Miner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("Bitcoin69Miner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateBitcoin69s(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&Bitcoin69Miner, boost::cref(chainparams)));
    }
