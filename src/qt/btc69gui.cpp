...
const std::string Bitcoin69GUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString Bitcoin69GUI::DEFAULT_WALLET = "~Default";

Bitcoin69GUI::Bitcoin69GUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void Bitcoin69GUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a btc69 address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and btc69: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to btc69 addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your btc69 addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified btc69 addresses"));
    ...
    openAction->setStatusTip(tr("Open a Bitcoin69: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible btc69 command-line options").arg(tr(PACKAGE_NAME)));
...
void Bitcoin69GUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void Bitcoin69GUI::createToolBars()
{
    if(walletFrame)
    {
        /** BTC69 START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/btc69cointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* btc69LabelLayout = new QVBoxLayout(toolbarWidget);
        btc69LabelLayout->addWidget(label);
        btc69LabelLayout->addWidget(toolbar);
        btc69LabelLayout->setDirection(QBoxLayout::TopToBottom);
        btc69LabelLayout->addStretch(1);
        ...
        // Get the latest Bitcoin69 release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void Bitcoin69GUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool Bitcoin69GUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool Bitcoin69GUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void Bitcoin69GUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void Bitcoin69GUI::setWalletActionsEnabled(bool enabled)
{
...
void Bitcoin69GUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void Bitcoin69GUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void Bitcoin69GUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void Bitcoin69GUI::optionsClicked()
{
...
void Bitcoin69GUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void Bitcoin69GUI::showDebugWindow()
{
...
void Bitcoin69GUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void Bitcoin69GUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void Bitcoin69GUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void Bitcoin69GUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void Bitcoin69GUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void Bitcoin69GUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void Bitcoin69GUI::openClicked()
{
...
void Bitcoin69GUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void Bitcoin69GUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void Bitcoin69GUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void Bitcoin69GUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void Bitcoin69GUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void Bitcoin69GUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** BTC69 START */
void Bitcoin69GUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void Bitcoin69GUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void Bitcoin69GUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void Bitcoin69GUI::gotoRestrictedAssetsPage()
{
...
void Bitcoin69GUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to btc69 network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void Bitcoin69GUI::setNumConnections(int count)
{
    updateNetworkState();
}

void Bitcoin69GUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void Bitcoin69GUI::updateHeadersSyncProgressLabel()
{
...
void Bitcoin69GUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void Bitcoin69GUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Bitcoin69"); // default title
...
void Bitcoin69GUI::changeEvent(QEvent *e)
{
...
void Bitcoin69GUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void Bitcoin69GUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void Bitcoin69GUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "btc69")
        msg += tr("Amount: %1\n").arg(Bitcoin69Units::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(Bitcoin69Units::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void Bitcoin69GUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void Bitcoin69GUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool Bitcoin69GUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool Bitcoin69GUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void Bitcoin69GUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void Bitcoin69GUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void Bitcoin69GUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void Bitcoin69GUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void Bitcoin69GUI::detectShutdown()
{
...
void Bitcoin69GUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void Bitcoin69GUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void Bitcoin69GUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(Bitcoin69GUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(Bitcoin69GUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void Bitcoin69GUI::subscribeToCoreSignals()
{
...
void Bitcoin69GUI::unsubscribeFromCoreSignals()
{
...
void Bitcoin69GUI::toggleNetworkActive()
{
    if (clientModel) {
...
void Bitcoin69GUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<Bitcoin69Units::Unit> units = Bitcoin69Units::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const Bitcoin69Units::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(Bitcoin69Units::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(Bitcoin69Units::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (Bitcoin69Units::Unit u : Bitcoin69Units::availableUnits())
    {
        QAction *menuAction = new QAction(QString(Bitcoin69Units::name(u)), this);
...
{
    setText(Bitcoin69Units::name(newUnits));
}
...
void Bitcoin69GUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=btc69BTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void Bitcoin69GUI::mnemonic()
{
...
void Bitcoin69GUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/JustAResearcher/Bitcoin69/releases"));
