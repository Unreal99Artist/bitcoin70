...
#include "btc69addressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a btc69 address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new Bitcoin69AddressEntryValidator(parent));
    widget->setCheckValidator(new Bitcoin69AddressCheckValidator(parent));
}
...
bool parseBitcoin69URI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no btc69: URI
    if(!uri.isValid() || uri.scheme() != QString("btc69"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!Bitcoin69Units::parse(Bitcoin69Units::btc69, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitcoin69URI(QString uri, SendCoinsRecipient *out)
{
    // Convert btc69:// to btc69:
    //
    //    Cannot handle this later, because btc69:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("btc69://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "btc69:");
    }
    QUrl uriInstance(uri);
    return parseBitcoin69URI(uriInstance, out);
}

QString formatBitcoin69URI(const SendCoinsRecipient &info)
{
    QString ret = QString("btc69:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(Bitcoin69Units::format(Bitcoin69Units::btc69, info.amount, false, Bitcoin69Units::separatorNever));
        paramCount++;
    }
...
bool openBitcoin69Conf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BTC69_CONF_FILENAME));

    /* Open btc69.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoin69.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoin69 (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Bitcoin69 (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Bitcoin69*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "btc69.desktop";
    return GetAutostartDir() / strprintf("btc69-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a btc69.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Bitcoin69\n";
        else
            optionFile << strprintf("Name=Bitcoin69 (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the btc69 app
...
bool GetStartOnSystemStartup()
{
    CFURLRef btc69AppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (btc69AppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, btc69AppUrl);

    CFRelease(btc69AppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add btc69 app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, btc69AppUrl, nullptr, nullptr);
    }
