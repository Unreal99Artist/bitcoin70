...
The BTC69coin Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play btc69s" on the test network, if you
...
BTC69 Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- BTC69Miner : Generates btc69s (if wallet is enabled).
...
However, in open source software such as BTC69 Core, where everyone uses
...
pay attention to for reviewers of BTC69 Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BTC69 Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BTC69coin so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:btc69/btc69.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `btc69-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `btc69-cli` and the GUI debug console use this table to determine how to
